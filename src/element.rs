...
impl Element {
    ...
    /// ```
    pub fn builder<S: AsRef<str>, NS: Into<Option<String>>>(
        name: S,
        namespace: NS,
    ) -> ElementBuilder {
        ElementBuilder {
    ...
    /// assert_eq!(bare.text(), "");
    /// ```
    pub fn bare<S: Into<String>, NS: Into<Option<String>>>(name: S, namespace: NS) -> Element {
        Element::new(
            name.into(),
            namespace.into(),
            None,
    ...
    pub fn write_to_inner<W: Write>(&self, writer: &mut ItemWriter<W>) -> Result<()> {
        ...
        let namespace = if self.namespace.is_none() || self.namespace.as_ref().unwrap().len() == 0 {
            None
        } else {
            Some(Arc::new(self.namespace.clone().unwrap().try_into()?))
        };
